...
import { useFetchLendingRecords, useCreateLendingRecord, useEditLendingRecord } from '../../../hooks';
...
const RecordFormModal = () => {
	const { myProfile, members, categories, places, lendingRecords, dispatch } = useContext(RecordsContext);
		...
		(formValue) => {
			create(formValue)
				.then(() => {
					...
					fetchRecord().then(({ data }) => dispatch(recordActions.updateLendingRecords(data)));
				})
				.catch((e) => {
